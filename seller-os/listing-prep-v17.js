...
let state={candidates:[],selectedId:null,market:'TW',bulkMarkets:['TW','SG','MY','TH','PH','VN'],listingStatus:{environment:'unknown',connections:[]},categoryCache:{},attributeCache:{},brandCache:{},logisticsCache:{},publishAttemptCache:{},busy:false};
...
function selectedBulkMarkets(){
  return MARKETS.filter(m=>state.bulkMarkets.includes(m.code)&&!m.future).map(m=>m.code);
}
function fillDraftFromMaster(c,code){
  const p=plan(c,code),d=draft(c,code);
  d.enabled=true;
  if(!String(d.title||'').trim())d.title=String(c.name||'').trim();
  if(!String(d.description||'').trim())d.description=defaultDescription(c,code);
  if(!String(d.sku||'').trim())d.sku=baseSku(c,code);
  if(!(Number(d.priceLocal)>0)&&Number(p.plannedPrice)>0)d.priceLocal=Number(p.plannedPrice);
  if(!(Number(d.initialStock)>0)&&Number(c.initialUnits)>0)d.initialStock=Number(c.initialUnits);
  ...
  d.sourceSnapshot={candidateId:c.id,candidateName:c.name||'',sourceUrl:c.sourceUrl||'',supplier:c.supplier||'',purchaseCostJpy:Number(c.purchaseCostJpy||0),preparedAt:now(),mode:'MULTI_MARKET_MASTER'};
  touch(d);
  return d;
}
async function prepareBulkDrafts(){
  const c=candidate();
  if(!c)return flash('후보상품을 먼저 선택해줘.','warn');
  if(c.id==='SANDBOX-TEST-TW')return flash('Sandbox 테스트 후보가 아니라 실제 후보상품을 선택해줘.','warn');
  if(c.status!=='READY')return flash('READY 상태 후보상품만 일괄 준비할 수 있어.','warn');
  const codes=selectedBulkMarkets();
  if(!codes.length)return flash('일괄 준비할 국가를 하나 이상 선택해줘.','warn');
  if(state.busy)return;
  state.busy=true;
  try{
    codes.forEach(code=>fillDraftFromMaster(c,code));
    await saveCandidate(c,{quiet:true});
    renderAll();
    const missingPrice=codes.filter(code=>!(Number(draft(c,code).priceLocal)>0));
    const missingShop=codes.filter(code=>!draft(c,code).selectedShopId);
    let msg=`${codes.length}개 국가 등록초안을 한 번에 만들었어.`;
    if(missingPrice.length)msg+=` 판매가 미완성: ${missingPrice.join(', ')}.`;
    if(missingShop.length)msg+=` Shop 연결 확인 필요: ${missingShop.join(', ')}.`;
    flash(msg,missingPrice.length||missingShop.length?'warn':'ok');
  }catch(e){flash(e.message,'bad')}finally{state.busy=false}
}
function renderBulkControls(){
  const root=$('#bulkMarketGrid'),status=$('#bulkStatus');
  if(!root||!status)return;
  const c=candidate();
  root.innerHTML=MARKETS.map(m=>{
    const checked=state.bulkMarkets.includes(m.code);
    const d=c?draft(c,m.code):null;
    const connCount=connectionsFor(m.code).length;
    const priceReady=Boolean(d&&Number(d.priceLocal)>0);
    const disabled=m.future?'disabled':'';
    return `<label><input type="checkbox" class="bulkMarketCheck" value="${m.code}" ${checked&&!m.future?'checked':''} ${disabled}><span><b>${m.name}</b> <small>${m.code}</small> · Shop ${connCount} · 가격 ${priceReady?'✓':'-' }${m.future?' · 향후':''}</span></label>`;
  }).join('');
  $('.bulkMarketCheck').forEach(el=>el.onchange=()=>{
    const code=el.value;
    if(el.checked){if(!state.bulkMarkets.includes(code))state.bulkMarkets.push(code)}
    else state.bulkMarkets=state.bulkMarkets.filter(x=>x!==code);
    renderBulkControls();
  });
  const codes=selectedBulkMarkets();
  if(!c){status.textContent='후보상품을 선택해줘.';return}
  const ready=codes.filter(code=>clientReadiness(c,code).ready).length;
  const prices=codes.filter(code=>Number(draft(c,code).priceLocal)>0).length;
  const shops=codes.filter(code=>draft(c,code).selectedShopId).length;
  status.innerHTML=`선택 <b>${codes.length}</b>개국 · 판매가 준비 <b>${prices}/${codes.length}</b> · Shop 선택 <b>${shops}/${codes.length}</b> · 클라이언트 검사 가능 <b>${ready}/${codes.length}</b><br><span class="tiny">일괄생성 후에도 카테고리·필수속성·물류·이미지는 각 국가 Shopee 기준으로 확인해야 해. Production 일괄등록은 계속 잠겨 있어.</span>`;
}
...
function renderAll(){renderMetrics();renderCandidateSelect();renderHeader();renderBulkControls();renderMarketTabs();renderEditor();renderSummary()}
...
$('#bulkSelectAll')?.addEventListener('click',()=>{state.bulkMarkets=MARKETS.filter(m=>!m.future).map(m=>m.code);renderBulkControls()});
$('#bulkClear')?.addEventListener('click',()=>{state.bulkMarkets=[];renderBulkControls()});
$('#bulkPrepare')?.addEventListener('click',()=>prepareBulkDrafts());
$('#bulkExport')?.addEventListener('click',()=>{
  const c=candidate();if(!c)return flash('후보상품을 먼저 선택해줘.','warn');
  const codes=selectedBulkMarkets();if(!codes.length)return flash('내보낼 국가를 하나 이상 선택해줘.','warn');
  saveBlob(`japanova-${c.id}-selected-markets.json`,{schema:'JAPANOVA_MULTI_MARKET_BUNDLE_V1',generatedAt:now(),candidateId:c.id,candidateName:c.name,markets:codes,packages:Object.fromEntries(codes.map(code=>[code,buildPackage(c,code)]))});
});
