...
function listingBaseSku(c,code){const raw=String(c.id||c.name||'ITEM').toUpperCase().replace(/[^A-Z0-9_-]+/g,'-').replace(/^-+|-+$/g,'');return `JNV-${(raw||'ITEM').slice(-24)}-${code}`.slice(0,100)}
function listingDescription(c,code){
...
function prepareMarketDraft(c,code){
  const p=planOf(c,code),s=sourceMeta(c);
  p.decision='SELL';
  const d=(p.listingDraft&&typeof p.listingDraft==='object'&&!Array.isArray(p.listingDraft))?p.listingDraft:{};
  p.listingDraft=d;
  d.enabled=true;
  if(!String(d.title||'').trim())d.title=String(c.name||'').trim();
  if(!String(d.description||'').trim())d.description=listingDescription(c,code);
  if(!String(d.sku||'').trim())d.sku=listingBaseSku(c,code);
  if(Number(p.plannedPrice)>0)d.priceLocal=Number(p.plannedPrice);
  d.initialStock=Math.max(1,Math.floor(Number(c.initialUnits)||1));
  ...
  if(String(s.jan||'').trim()&&!String(d.gtin||'').trim())d.gtin=String(s.jan).trim();
  d.condition=d.condition||'NEW';
  d.sourceSnapshot={
    candidateId:c.id,
...
async function prepareAllListingDrafts(){
  let saved=await saveCandidate({quiet:true});if(!saved)return;
  const sellRows=candidateStats(saved).rows.filter(x=>x.final.key==='SELL');
  if(!sellRows.length)return flash('판매후보(SELL) 국가가 아직 없어. 규제와 판매가를 먼저 검증해줘.','warn');
  const prepared=[];
  for(const row of sellRows){
    if(row.m.future)continue;
    prepareMarketDraft(saved,row.m.code);
    prepared.push(row.m.code);
  }
  if(!prepared.length)return flash('등록초안을 만들 수 있는 판매후보 국가가 없어.','warn');
  saved.status='READY';
  saved=await persistCandidate(saved,{silent:true});
  state.selectedId=saved.id;renderAll();fillForm(saved);
  flash(`판매후보 ${prepared.length}개국 등록초안을 만들었어: ${prepared.map(code=>meta(code).name).join(', ')}. 국가별 Shopee 카테고리·속성·물류·이미지는 등록 준비 화면에서 각각 확인해줘.`);
}
