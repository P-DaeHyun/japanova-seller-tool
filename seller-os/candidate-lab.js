...
let state={candidates:[],selectedId:null,calcData:null,fx:null};
...
function loadLocal(){try{const raw=JSON.parse(localStorage.getItem(STORAGE_KEY)||'[]');state.candidates=Array.isArray(raw)?raw:[]}catch{state.candidates=[]}}
...
async function loadReference(){
  const [calc,fx]=await Promise.all([
    ...
    fetch(`${API}/api/fx`).then(r=>{if(!r.ok)throw new Error('환율 API 오류');return r.json()}).catch(async()=>{
      const r=await fetch('../fx.json',{cache:'no-store'});if(!r.ok)throw new Error('환율 데이터를 불러오지 못했습니다.');return r.json();
    })
  ]);
...
function analyzeSummary(c){
  let pass=0,loss=0,planned=0;
  for(const m of MARKET_META.filter(x=>x.active)){
    const e=evaluatePlan(c,m.code);if(!e)continue;planned++;if(!e.error&&e.profitJpy<0)loss++;if(!e.error&&e.profitJpy>=0&&e.margin>=Number(c.targetMarginPct||20)/100)pass++;
  }
  return {pass,loss,planned};
}

function renderMetrics(){
  const total=state.candidates.length;
  const ready=state.candidates.filter(x=>x.status==='READY').length;
  let loss=0,passMarkets=0,cash=0;
  for(const c of state.candidates){const s=analyzeSummary(c);loss+=s.loss;passMarkets+=s.pass;cash+=Number(c.purchaseCostJpy||0)*Math.max(1,Number(c.initialUnits||1));}
  $('#mCandidates').textContent=fmt(total);$('#mReady').textContent=fmt(ready);$('#mLoss').textContent=fmt(loss);$('#mPass').textContent=fmt(passMarkets);$('#mCash').textContent=jpy(cash);
}

function renderList(){
  const body=$('#candidateRows');
  if(!state.candidates.length){body.innerHTML='<tr><td colspan="7" class="empty">아직 후보상품이 없어. 오른쪽에서 첫 후보를 만들어도 돼.</td></tr>';return}
  body.innerHTML=state.candidates.map(c=>{
    const s=analyzeSummary(c);const status=c.status==='READY'?'<span class="ok">등록 후보</span>':c.status==='HOLD'?'<span class="warn">보류</span>':'조사중';
    return `<tr class="${c.id===state.selectedId?'sel':''}"><td><button class="link" data-open="${esc(c.id)}">${esc(c.name||'(이름 없음)')}</button></td><td>${jpy(c.purchaseCostJpy)}</td><td>${fmt(c.weightG)}g</td><td>${status}</td><td class="ok">${s.pass}</td><td class="${s.loss?'bad':'muted'}">${s.loss}</td><td><button class="mini" data-open="${esc(c.id)}">분석</button></td></tr>`;
  }).join('');
  body.querySelectorAll('[data-open]').forEach(b=>b.onclick=()=>openCandidate(b.dataset.open));
}

function fillForm(c){
  $('#formTitle').textContent=c?'후보상품 편집':'새 후보상품';
  const x=c||blankCandidate();
  setVal('#cName',x.name);setVal('#cSource',x.sourceUrl);setVal('#cSupplier',x.supplier);setVal('#cPurchase',x.purchaseCostJpy);setVal('#cPack',x.packagingCostJpy);setVal('#cDomestic',x.domesticShippingJpy);setVal('#cOther',x.otherCostJpy);setVal('#cWeight',x.weightG);setVal('#cL',x.lengthCm);setVal('#cW',x.widthCm);setVal('#cH',x.heightCm);setVal('#cDiscount',x.discountPct);setVal('#cPayoneer',x.payoneerPct);setVal('#cFxBuffer',x.fxBufferPct);setVal('#cTarget',x.targetMarginPct);setVal('#cUnits',x.initialUnits);setVal('#cNote',x.note);setVal('#cStatus',x.status||'RESEARCH');
  $('#candidateId').value=c?.id||'';
}
function readForm(){
  const existing=selected();const id=$('#candidateId').value||uid();
  return {...(existing||{}),id,name:$('#cName').value.trim(),sourceUrl:$('#cSource').value.trim(),supplier:$('#cSupplier').value.trim(),purchaseCostJpy:num('#cPurchase'),packagingCostJpy:num('#cPack'),domesticShippingJpy:num('#cDomestic'),otherCostJpy:num('#cOther'),weightG:Math.max(1,num('#cWeight')),lengthCm:num('#cL'),widthCm:num('#cW'),heightCm:num('#cH'),discountPct:num('#cDiscount'),payoneerPct:num('#cPayoneer'),fxBufferPct:num('#cFxBuffer'),targetMarginPct:num('#cTarget'),initialUnits:Math.max(1,num('#cUnits')),status:$('#cStatus').value,note:$('#cNote').value.trim(),plans:existing?.plans||{},createdAt:existing?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
}
function saveCandidate(){const c=readForm();if(!c.name){flash('후보상품 이름을 입력해줘.','warn');return}if(!(c.purchaseCostJpy>0)){flash('매입원가는 0보다 커야 해.','warn');return}const i=state.candidates.findIndex(x=>x.id===c.id);if(i>=0)state.candidates[i]=c;else state.candidates.unshift(c);state.selectedId=c.id;saveLocal();renderAll();flash('후보상품과 분석조건을 저장했어.')}
...
function deleteCandidate(){const c=selected();if(!c)return;if(!confirm(`“${c.name}” 후보를 삭제할까?`))return;state.candidates=state.candidates.filter(x=>x.id!==c.id);state.selectedId=null;saveLocal();renderAll();newCandidate();flash('후보상품을 삭제했어.')}

function renderAnalysis(){
  const c=selected();if(!c){$('#analysis').innerHTML='<div class="empty">왼쪽 후보상품을 선택해.</div>';return}
  const target=Number(c.targetMarginPct||20)/100;
  const rows=MARKET_META.map(m=>{
    const a=assumptionFor(m.code),fx=baseFxFor(m.code);const plan=c.plans?.[m.code]||{};
    if(!a||!fx){return `<tr><td>${m.name}<div class="muted">${m.code} · ${m.cur}</div></td><td>${fx?fmt(fx,6):'-'}</td><td colspan="5" class="warn">수수료/SLS 기준 미확정 · 계산 보류</td><td><input class="plan" data-code="${m.code}" data-field="competitorPrice" type="number" step="any" value="${esc(plan.competitorPrice||'')}" placeholder="경쟁가"></td><td colspan="3" class="muted">브라질은 확정 데이터가 생기면 자동 계산 대상에 포함</td></tr>`}
    const b0=solve(c,m.code,0),b10=solve(c,m.code,.10),b20=solve(c,m.code,.20),b30=solve(c,m.code,.30),ev=evaluatePlan(c,m.code),rk=risk(ev,target);
    const shipping=(!b20||b20.error)?'-':jpy(b20.shippingJpy);
    return `<tr><td><b>${m.name}</b><div class="muted">${m.code} · ${m.cur}</div></td><td>${fmt(fx,fx<0.1?6:4)}</td><td>${shipping}</td><td>${b0?.error?'<span class="bad">오류</span>':local(roundLocal(b0?.listedLocal,m.cur),m.cur)}</td><td>${b10?.error?'-':local(roundLocal(b10?.listedLocal,m.cur),m.cur)}</td><td class="target">${b20?.error?'-':local(roundLocal(b20?.listedLocal,m.cur),m.cur)}</td><td>${b30?.error?'-':local(roundLocal(b30?.listedLocal,m.cur),m.cur)}</td><td><input class="plan" data-code="${m.code}" data-field="competitorPrice" type="number" step="any" value="${esc(plan.competitorPrice||'')}" placeholder="경쟁가"></td><td><input class="plan" data-code="${m.code}" data-field="plannedPrice" type="number" step="any" value="${esc(plan.plannedPrice||'')}" placeholder="내 판매가"></td><td class="${ev&&!ev.error&&ev.profitJpy<0?'bad':'money'}">${ev&&!ev.error?jpy(ev.profitJpy):'-'}</td><td class="${rk.cls}">${ev&&!ev.error?pct(ev.margin):'-'}<div>${rk.label}</div></td></tr>`;
  }).join('');
  const totalCost=Number(c.purchaseCostJpy)+Number(c.packagingCostJpy)+Number(c.domesticShippingJpy)+Number(c.otherCostJpy);
  const cash=Number(c.purchaseCostJpy||0)*Math.max(1,Number(c.initialUnits||1));
  ...
  $('#analysis').querySelectorAll('.plan').forEach(inp=>inp.onchange=()=>{const cur=selected();if(!cur)return;cur.plans=cur.plans||{};cur.plans[inp.dataset.code]=cur.plans[inp.dataset.code]||{};cur.plans[inp.dataset.code][inp.dataset.field]=Number(inp.value||0)||0;cur.updatedAt=new Date().toISOString();saveLocal();renderAnalysis();renderMetrics();renderList()});
}
...
function importJson(file){const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);const arr=Array.isArray(d)?d:d.candidates;if(!Array.isArray(arr))throw new Error('candidates 배열이 없습니다.');state.candidates=arr;state.selectedId=null;saveLocal();renderAll();newCandidate();flash(`${arr.length}개 후보상품을 복원했어.`)}catch(e){flash(`복원 실패: ${e.message}`,'bad')}};r.readAsText(file)}
...
async function boot(){
  loadLocal();newCandidate();renderMetrics();renderList();
  try{await loadReference();$('#status').textContent='분석엔진 정상';$('#status').className='badge ok';renderAll()}catch(e){$('#status').textContent='분석데이터 오류';$('#status').className='badge bad';flash(e.message,'bad')}
