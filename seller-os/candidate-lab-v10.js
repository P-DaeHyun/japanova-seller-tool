...
async function loadCandidates(){
  ...
  try{
    ...
    state.candidates=server;saveLocal();$('#status').textContent='v1.0 검증엔진 · DB 저장';$('#status').className='badge ok';
  }catch{state.dbStorage=false;state.candidates=cached;$('#status').textContent='v1.0 검증엔진 · 로컬 백업';$('#status').className='badge warn'}
}
async function persistCandidate(c,{silent=false}={}){
  ...
  catch(e){state.dbStorage=false;$('#status').textContent='v1.0 검증엔진 · 로컬 백업';$('#status').className='badge warn';if(!silent)flash(`DB 저장 실패, 브라우저에 백업 중: ${e.message}`,'warn');return c}
}
...
function fillForm(c){const x=c||blankCandidate();$('#formTitle').textContent=c?'후보상품 편집':'새 후보상품';setVal('#cName',x.name);setVal('#cSource',x.sourceUrl);setVal('#cSupplier',x.supplier);setVal('#cPurchase',x.purchaseCostJpy);setVal('#cPack',x.packagingCostJpy);setVal('#cDomestic',x.domesticShippingJpy);setVal('#cOther',x.otherCostJpy);setVal('#cWeight',x.weightG);setVal('#cUnits',x.initialUnits);setVal('#cL',x.lengthCm);setVal('#cW',x.widthCm);setVal('#cH',x.heightCm);setVal('#cDiscount',x.discountPct);setVal('#cPayoneer',x.payoneerPct);setVal('#cFxBuffer',x.fxBufferPct);setVal('#cTarget',x.targetMarginPct);setVal('#cStatus',x.status||'RESEARCH');setVal('#cNote',x.note);$('#candidateId').value=c?.id||''}
function readForm(){const old=selected(),id=$('#candidateId').value||uid();return {...(old||{}),id,name:$('#cName').value.trim(),sourceUrl:$('#cSource').value.trim(),supplier:$('#cSupplier').value.trim(),purchaseCostJpy:num('#cPurchase'),packagingCostJpy:num('#cPack'),domesticShippingJpy:num('#cDomestic'),otherCostJpy:num('#cOther'),weightG:Math.max(1,num('#cWeight')),initialUnits:Math.max(1,num('#cUnits')),lengthCm:num('#cL'),widthCm:num('#cW'),heightCm:num('#cH'),discountPct:num('#cDiscount'),payoneerPct:num('#cPayoneer'),fxBufferPct:num('#cFxBuffer'),targetMarginPct:num('#cTarget'),status:$('#cStatus').value,note:$('#cNote').value.trim(),plans:old?.plans||{},createdAt:old?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()}}
async function saveCandidate(){const c=readForm();if(!c.name)return flash('후보상품 이름을 입력해줘.','warn');if(!(c.purchaseCostJpy>0))return flash('매입원가는 0보다 커야 해.','warn');const i=state.candidates.findIndex(x=>x.id===c.id);if(i>=0)state.candidates[i]=c;else state.candidates.unshift(c);state.selectedId=c.id;const saved=await persistCandidate(c);state.selectedId=saved.id;renderAll();fillForm(saved);flash(state.dbStorage?'후보상품과 검증조건을 DB에 저장했어.':'후보상품을 로컬 백업에 저장했어.')}
...
async function boot(){
  ...
  catch(e){$('#status').textContent='초기화 오류';$('#status').className='badge bad';flash(e.message,'bad')}
}
$('#navLab').onclick=()=>showView('lab');$('#navCompare').onclick=()=>showView('compare');$('#navOps').onclick=()=>showView('ops');$('#newCandidate').onclick=newCandidate;$('#saveCandidate').onclick=saveCandidate;$('#deleteCandidate').onclick=deleteCandidate;$('#exportCandidates').onclick=exportCandidates;$('#importFile').onchange=e=>{if(e.target.files?.[0])importCandidates(e.target.files[0])};
boot();
})();
